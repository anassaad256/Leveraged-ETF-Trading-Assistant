"""
Leveraged ETF Trading Analyzer

Implements a crash-aware, pre-crash 6-month quintile mean-reversion framework
for daily 2x leveraged ETFs.
"""
...
# Well-known 2x leveraged ETF tickers
KNOWN_2X_ETFS = {
...
def is_likely_2x_leveraged(ticker: str) -> bool:
    """Check if ticker is a known 2x (or 3x) leveraged ETF."""
...
def detect_crash(df: pd.DataFrame, decline_threshold: float = 0.17,
    ...
    Criteria (any triggers crash):
      1. Peak-to-trough decline >= threshold within <= window_days trading days
      2. Persistent lower lows with expanding volatility
      3. Rapid downward acceleration with gap behaviour
    """
    closes = df["close"].values
    dates = df.index

    # --- Criterion 1: rolling peak-to-trough ---
    ...
    # --- Criterion 2: persistent lower lows + expanding vol ---
    ...
    # --- Criterion 3: rapid downward acceleration / gaps ---
    if not crash_detected:
        if len(closes) >= 10:
...
def build_reference_window(df: pd.DataFrame, crash_info: dict) -> dict:
    """Select and return the reference window DataFrame + metadata."""
...
def compute_quintiles(ref_window: dict) -> dict:
    """Compute quintile boundaries from the reference window."""
    ...
    for q in range(1, 6):
        ...
        quintiles[q] = {"lower": round(lower, 2), "upper": round(upper, 2)}

    return {
        "ref_high": round(ref_high, 2),
        "ref_low": round(ref_low, 2),
        "band_width": round(band, 2),
        "quintiles": quintiles,
    }


def assess_current_price(current_price: float, quintile_data: dict,
                         crash_info: dict) -> dict:
    """Determine which quintile the current price falls into."""
    q = quintile_data["quintiles"]
    position = None
    quintile_num = None

    if current_price < q[1]["lower"]:
        if crash_info["detected"]:
            position = "Below 1st Quintile (Dislocation/Overshoot)"
        else:
            position = "Below 1st Quintile"
        quintile_num = 0
        ...
        for i in range(1, 6):
            if q[i]["lower"] <= current_price <= q[i]["upper"]:
                labels = {
                    1: "1st Quintile (Cheapest)",
                    2: "2nd Quintile (Below Average)",
                    3: "3rd Quintile (Neutral)",
                    4: "4th Quintile (Expensive)",
    ...
    return {
        ...
        "quintile_num": quintile_num,
        "position": position,
        "ref_high": quintile_data["ref_high"],
        "ref_low": quintile_data["ref_low"],
    }


def generate_buy_recommendation(price_assessment: dict,
                                quintile_data: dict) -> dict:
    """Generate buy/wait recommendation with entry plan."""
    q = quintile_data["quintiles"]
    qn = price_assessment["quintile_num"]
    price = price_assessment["current_price"]

    entry_plan = {
        "upper_1st": round(q[1]["upper"], 2),
    ...
    if qn == 0:
        recommendation = "BUY"
        status = (
            "Price is below the 1st quintile — panic overshoot territory. "
            "Staggered entries recommended with awareness of further downside."
        )
    elif qn == 1:
        recommendation = "BUY"
        status = (
            "Price is in the 1st quintile (cheapest zone). "
            "Mean-reversion setup is active. Use staggered entries."
        )
    elif qn == 2:
        recommendation = "WAIT"
        status = (
            f"Price is in the 2nd quintile. Not optimal yet. "
        ...
        status = (
            f"Price is in the {label} quintile. Do not buy. "
            f"Best buy occurs if price enters the 1st quintile near ${entry_plan['upper_1st']:.2f}."
        )
    ...
    return {
        ...
        "status": status,
        "entry_plan": entry_plan if show_entry else None,
        "target_entry": entry_plan,
    }


def generate_exit_recommendation(price_assessment: dict,
                                 quintile_data: dict) -> dict:
    """Generate exit zones and recommendation."""
    q = quintile_data["quintiles"]
    qn = price_assessment["quintile_num"]

    exit_zones = {
        "partial_exit": round(q[3]["lower"], 2),
    ...
    if qn >= 5 or qn == 6:
        ...
        exit_note = (
            "Strongly discourage holding — volatility decay risk is elevated. "
            "Full exit recommended immediately."
        )
    elif qn == 4:
        exit_recommendation = "SELL"
        exit_note = (
            "Full or near-full exit recommended. Asymmetry is no longer favorable."
        )
    elif qn == 3:
        exit_recommendation = "PARTIAL SELL"
        exit_note = (
            "Consider partial profit-taking. Asymmetry is diminishing in mid-to-late 3rd quintile."
        )
    else:
...
def assess_risk(price_assessment: dict, crash_info: dict,
                quintile_data: dict) -> str:
    """Return the primary risk statement."""
    qn = price_assessment["quintile_num"]

    if qn >= 4:
        return (
            "Volatility decay: Holding a 2x leveraged ETF in the upper quintiles "
    ...
    if qn == 0 and crash_info["detected"]:
        return (
            "Regime break risk: Price has broken below the pre-crash reference range. "
    ...
    if qn <= 1 and crash_info["detected"]:
        return (
            "Drawdown risk: In a crash regime, prices can continue falling further "
    ...
    if qn <= 1:
        return (
            "Drawdown risk: While in the cheapest quintile, further decline is possible. "
    ...
    return (
        "Path dependency: 2x leveraged instruments are designed for daily rebalancing. "
...
def generate_summary(buy_rec: dict, exit_rec: dict, price_assessment: dict,
                     crash_info: dict) -> dict:
    """Produce the final summary and recommendation."""
    # Exit rules have higher priority than entry rules
    ...
    if final == "SELL":
        text = (
            f"At ${price:.2f}, the ETF is in an expensive zone. "
            "Exit positions to avoid volatility decay and unfavorable asymmetry. "
        )
    elif final == "PARTIAL SELL":
        text = (
            f"At ${price:.2f}, the ETF is in neutral territory. "
        ...
        text = (
            f"At ${price:.2f}, the ETF is in the cheapest zone offering "
            f"favorable mean-reversion asymmetry. Use staggered entries.{crash_note}"
        )
    else:
        text = (
            f"At ${price:.2f}, the ETF is not yet in the optimal buy zone. "
...
def analyze_ticker(ticker: str) -> dict:
    ...
    if len(df) < 30:
        raise ValueError(
            f"Insufficient historical data for '{ticker}' ({len(df)} days). "
